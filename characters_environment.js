...
//Agregar las variables de la posicion de la nariz
noseX = "";
noseY = "";
GameStatus = "";

// se define la funcion de arranque antes para mostrar el estado de carga del juego
function startGame()
{
  GameStatus = "start";
...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    //--------------CAMBIAR ESTA LINEA EN LA C142------------------------------- 
    // ANTES ----- text("Presiona cualquier flecha del teclado para iniciar el juego", gameConfig.screenX/2, gameConfig.screenY/2);
...
//deben cumplirse las 3 condiciones, tecla jugar presionada, estatus de carga y deteccion de nariz
 if(noseX !="" && gameConfig.status==="start" && GameStatus=="start")
 { 
   document.getElementById("status").innerHTML = "El juego se ha cargado";

   //solo suena la musica de fondo al arrancar el juego
   world_start.play();
...
function manualControl(character){
  ...
  if(character.live){
    ...
    // cambiar control con flechas por control con nariz en X y Y
    if(noseX < 300){
      character.velocity.x-=gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(-1);
